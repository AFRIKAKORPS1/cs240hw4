...
void sortStrings(char strings[NUM_STRINGS][STRING_LENGTH])
{
	...
	for (int i = 0; i < NUM_STRINGS; i++)
	{
		...
		{
			if (strcmp(test[min], test[j]) > 0)
			{
				min = j; // set new min index
			}
		}

		swapStrings(test[min], test[i]); // swap strings
	}
}
...
int vowelCounter(char string[STRING_LENGTH])
{
	...
	while (*vcheck != '\0')
	{
		if (*vcheck == 'a'|| *vcheck == 'e'|| *vcheck == 'i'|| *vcheck == 'o'|| *vcheck == 'u' || *vcheck == 'A' || *vcheck == 'E' || *vcheck == 'I' || *vcheck == 'O' || *vcheck == 'U')
		{
			vcount++;
		}
		vcheck++;
